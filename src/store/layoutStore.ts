...
import { asyncRouteConfig, constantRouteConfig } from '@config/router.config';
import { userStore } from '@store/userStore';
import { localeStore } from '@store/localeStore';
import { Breadcrumb, RouteChild, RouteRoot, StoreKeyValue } from '@models/global';
...
class LayoutStore {
  ...
  async createLessScriptAndLink() {
    ...
    script.onload = () => {
      runInAction(() => {
        const status = localStorage.getItem('RCS-layoutStatus');
        // 判断是否需要变更主题
        if (status) {
          ...
          if (
            ...
            message.loading(localeObj['layoutSetting.changeCurrentTheme'] || '테마 적용', 3);
            setTimeout(() => {
  ...
  @action changeLayoutStatus = (key: keyof LayoutStatus, value: any) => {
    ...
    window.localStorage.setItem('RCS-layoutStatus', JSON.stringify(this.layoutStatus));
  };
