...
import { readSeasonTicketObj, readTicketObj } from '@utils/readFromCsv';
...
class Ticket extends PureComponent<any, IState> {
  ...
  async handleDownloadClick() {
    ...
    initialData.ticketType = '유료정기권';
    initialData.ticketName = '정기권명(정확히기입)';
    initialData.name = '김아무개';
    initialData.tel = '010-0000-0000';
    initialData.effectDate = '2021-01-01';
    initialData.expireDate = '2021-01-31';
    ...
    initialData.payMethod = '현금/카드(정확히기입)';
    initialData.extend = '사용/미사용(정확히기입)';
    ...
    this.state.list.map((ticket) => {
      ...
      data.payMethod = ticket.payMethod
        ? conversionEnumValue(ticket.payMethod, payMethodOpt).label
        : '';
      data.extend = ticket.extendYn ? conversionEnumValue(ticket.extendYn, useYnOpt).label : '';
      data.lastInDate =
  ...
  handleUploadData = (file: any): boolean => {
    if (file !== undefined) {
      ...
      fileReader.onload = () => {
        ...
        const parsedPassengerData: ITicketObj[] = readSeasonTicketObj(
          rawData,
          this.state.ticketClassesSelect
        );
        console.log(' csv data ' + JSON.stringify(parsedPassengerData));
        createSeasonTickets(parsedPassengerData)
          .then((res: any) => {
            const { msg, data } = res;
            if (msg === 'success') {
              runInAction(() => {
